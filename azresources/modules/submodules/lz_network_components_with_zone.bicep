...
// [Change Log]
// - 2025-01-23: MAC Only
//               Changed Network Security Groups
//               - from ${network.name}-${toLower(subnet.name)}-${loc}-nsg
//               - to   ${network.zone}-${network.name}-${toLower(subnet.name)}-${loc}-nsg
//               Changed User Defined Routes
//               - from ${network.name}-${toLower(subnet.name)}-${loc}-udr
//               - to   ${network.zone}-${network.name}-${toLower(subnet.name)}-${loc}-udr
// [Change Log - LZ_NETWORK_COMPONENTS_WITH_ZONE.BICEP]
...
module avm_routeTable 'br/public:avm/res/network/route-table:0.4.0' = [for subnet in network.subnets: if (subnet.udr.enabled) {
  ...
  params: {
    // Required parameters
    name: '${network.zone}-${network.name}-${toLower(subnet.name)}-${loc}-udr'
...
module avm_networkSecurityGateway 'br/public:avm/res/network/network-security-group:0.5.0' = [for subnet in network.subnets: if (subnet.nsg.enabled) {
  ...
  params: {
    // Required parameters
    name: '${network.zone}-${network.name}-${toLower(subnet.name)}-${loc}-nsg'
