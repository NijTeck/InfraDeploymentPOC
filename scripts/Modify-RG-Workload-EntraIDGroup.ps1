...
(
    ...
    [String]$Subscription,
    [Parameter(Mandatory=$true,
    HelpMessage="Enter Workload Name")]
    [String]$WorkloadName,
    [Parameter(Mandatory=$true,
...
try {
    ...
    $Subscription
    $WorkloadName = $WorkloadName.ToLower()
    $WorkloadName
    $Role = $Role.ToLower()
    $Role
    $DisplayName = "RG-$Subscription-$WorkloadName-$($Role)s".Replace(' ','-').ToLower()
    $DisplayName
    ...
    {
        $Description = "Purpose: $WorkloadName (resource group)rbac assignment / privileged identity management (No members)"
    }
    else
    {
        $Description = "Purpose: $WorkloadName (resource group) rbac assignment"
    }
    $Description
